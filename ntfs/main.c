...
void cmd_read(ntfs_info *ntfs, int argc, char *argv[])
{
#if 0
	char *path = argv[0];
...
void cmd_ls(ntfs_info *ntfs, int argc, char *argv[])
{
#if 0	
	char *path = argv[0];	
	directory_entry *d = path_to_entry(ext2, path);
	...
	if (ntfs)
	{
		if (num_sub_args)
		{
			if (strcmp(sub_argv[0], "ls") == 0) {
				cmd_ls(ntfs, num_sub_args-1, sub_argv+1);
			} else if (strcmp(sub_argv[0], "read") == 0) {
				cmd_read(ntfs, num_sub_args-1, sub_argv+1);
			} else {
				printf("unknown command: %s!\n", sub_argv[0]);
			}				
		} else {
			printf("ntfs_mount succeeded, but no commands provided\n");
		}
